...
- To run maven compile: 
  ...
  - Then double click 'Maven Build'. It will prompt you for the configuration. Enter the below:
    ...
    - Under Maven Runtime: (scroll down to see this option) we will be adding the location of the existing mvn install (its runs faster than using the embedded one)
